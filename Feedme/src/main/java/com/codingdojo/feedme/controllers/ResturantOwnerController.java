...
public class ResturantOwnerController {
  ...
  public String ownerProfile(HttpSession session, @PathVariable("newUserId") Long id, Model model) {
    ...
    // if the newly registered person does not have a resturant, we need to link him
    // to a form to create a new resturnt
  ...
  public String editProfile(@Valid @ModelAttribute("editRest") Resturant editRest, BindingResult result, Model model,HttpSession session) {
    ...
    if (result.hasErrors()) {
      ...
      Long id = thisUser.getOwner().getResturant().getId();
  ...
  public String showOrders(HttpSession session, Model model) {
    ...
    // this returns the orders that are not yet dilivered from the resturant side,
    // and since the resturant and owner are connected there is no problem of
    // resuing the same query
  ...
  public String handleaddMenuItems(@Valid @ModelAttribute("menuItem") MenuItem menuItem, BindingResult result,
